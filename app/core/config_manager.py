...
class ConfigManager:
    ...
    @staticmethod
    def construir_camera_info(cam: Dict[str, Any], nome_condominio: str, config_global: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        if config_global:
            return {
                ...
                "codigomaquina": cam.get("codigomaquina", config_global.get("codigomaquina", 1)),
                "codigoconjuntodeocorrencias": config_global.get("codigoconjuntodeocorrencias", 1),
                "setor": cam.get("setor", 1),
                "complemento": cam.get("complemento", f"{cam.get('name', 'CAMERA')} está offline"),
            ...
            return {
                ...
                "codigoconjuntodeocorrencias": cam.get("codigoconjuntodeocorrencias", 1),
                "setor": cam.get("setor", 1),
                "complemento": cam.get("complemento", f"{cam.get('name', 'CAMERA')} está offline"),
                "nome": cam.get("name", "CAMERA")
            }
    ...
    def extrair_config_global(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        """
        if "cliente" in data and "particao" in data:
            return {
