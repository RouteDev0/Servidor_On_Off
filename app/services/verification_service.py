...
class VerificationService:
    ...
    def verificar_camera_individual(self, cam: Dict[str, Any], nome_condominio: str, config_global: Optional[Dict[str, Any]] = None) -> tuple[str, str]:
        ...
        resultado_encontrado, resultado_cache = self.cache_manager.get_cached_result(chave_cache)
        if resultado_encontrado:
            ...
            if estado_anterior is False and resultado_cache:
                cam_info = ConfigManager.construir_camera_info(cam, nome_condominio, config_global)
        ...
        try:
            ...
            online = resp.status_code == 200 and resp.headers.get("Content-Type", "").startswith("image")
    ...
    def verificar_cameras(self, cameras: List[Dict[str, Any]], nome_condominio: str = "Condomínio", config_global: Optional[Dict[str, Any]] = None):
        ...
        print(f"[INFO] Verificando {num_cameras} câmeras em {nome_condominio} com {max_workers} workers e delay de {delay_entre_cameras}s")

        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = []
            for cam in cameras:
                futures.append(executor.submit(self.verificar_camera_individual, cam, nome_condominio, config_global))
                time.sleep(delay_entre_cameras)  # Pequeno delay entre submissões para não sobrecarregar a rede
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    if status_str != "NO_RTSP":
                        self.status_atual[nome_condominio].append({
                            "nome": nome,
                            "status": status_str
                        })
                except Exception as e:
